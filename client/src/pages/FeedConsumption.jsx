...
const COLUMNS = [
  ...
  { key: "qualityConsumed", label: "Quality Consumed" },
...
export default function FeedConsumption() {
  ...
  const fetchData = async () => {
    setLoading(true);
    const res = await feedConsumptionAPI.getAll();
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    if (editId) {
      const res = await feedConsumptionAPI.update(editId, form);
      if (res.success) setRows((p) => p.map((r) => r._id === editId ? res.data : r));
    } else {
      const res = await feedConsumptionAPI.create(form);
  ...
  const handleDelete = async (id) => {
    ...
    const res = await feedConsumptionAPI.remove(id);
    ...
    <div className="it-page">
      ...
      <div className="it-main">
        ...
        <div className="it-breadcrumb">
          ...
          <span className="it-bread-current">FEED CONSUMPTION</span>
        </div>
        <div className="it-toolbar">
          <button className="it-add-btn" onClick={openAdd}>+ Add New Consumption</button>
        </div>
        ...
        {showModal && (
          <div className="it-overlay" onClick={() => setShowModal(false)}>
            <div className="it-modal" onClick={(e) => e.stopPropagation()}>
              <h3 className="it-modal-title">{editId ? "Edit Consumption" : "Add New Consumption"}</h3>
