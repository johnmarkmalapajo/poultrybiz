...
const COLUMNS = [
  ...
  { key: "noOfBirds",             label: "Number of Birds",   type: "number" },
  { key: "nameOfVaccine",         label: "Vaccine/Drug" },
  { key: "targetAge",             label: "Target Age" },
  { key: "routeOfAdministration", label: "Route of Admin" },
  { key: "dosageFrequency",       label: "Dosage" },
...
export default function HealthRecord() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    if (editId) {
      const res = await healthAPI.update(editId, form);
      if (res.success) setRows((p) => p.map((r) => r._id === editId ? res.data : r));
    } else {
      const res = await healthAPI.create(form);
  ...
  const handleDelete = async (id) => {
    ...
    const res = await healthAPI.remove(id);
    ...
    <div className="rt-page">
      ...
      <div className="rt-main">
        ...
        <div className="rt-breadcrumb">
          ...
          <span className="rt-bread-current">HEALTH RECORD</span>
        </div>
        <div className="rt-toolbar">
          <button className="rt-add-btn" onClick={openAdd}>+ Add New Health Record</button>
        </div>
        ...
        {showModal && (
          <div className="rt-overlay" onClick={() => setShowModal(false)}>
            <div className="rt-modal" onClick={(e) => e.stopPropagation()}>
              <h3 className="rt-modal-title">{editId ? "Edit Health Record" : "Add New Health Record"}</h3>
